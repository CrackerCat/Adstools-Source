...
public class SignerApk implements Runnable {
    ...
    public void sign() {
        try {
            ...
            ProcessBuilder localProcessBuilder = new ProcessBuilder(new String[]{"cmd.exe", "/c", "java -Xmx1024m -jar \"" + str1 + "\\apksigner.jar\" sign --ks \"" + str1 + "\\Sign.jks\" --ks-pass pass:adstools --v4-signing-enabled false --out \"" + this.apkPath + "\" \"" + this.output + "\""});

            System.out.println("java -Xmx1024m -jar \"" + str1 + "\\apksigner.jar\" --ks \"" + str1 + "\\Sign.jks\" --ks-pass pass: --out \"" + this.apkPath + "\" \"" + this.output + "\"");
    ...
    public void zipalign() {
        try {
            ...
            System.out.println("\"" + str1 + "\\zipalign.exe\" -f -v 4 " + this.apkPath + " " + this.apkPath);
            localProcessBuilder.redirectErrorStream(true);
            Process localProcess = localProcessBuilder.start();
            BufferedReader localBufferedReader = new BufferedReader(new InputStreamReader(localProcess.getInputStream()));
            for (; ; ) {
